...
if ( ! function_exists('get_html_img'))
{
    ...
    function get_html_img($img_path, $width = 60, $height = 60,
                          $thumb_marker = 'small_thumb'
    )
    {
        if ( ! $img_path)
        {
        ...
        return '<img src="' . $full_path . '" width="' . $width . '" height="'
            . $height . '"/>';
    }
...
if ( ! function_exists('is_video_link'))
{
    ...
    {
        $info = pathinfo($field);
        if ($info["extension"] == 'mp4' || $info["extension"] == 'MP4'
            || $info["extension"] == 'flv'
            || $info["extension"] == 'FLV'
        )
        {
...
if ( ! function_exists('get_filter_img'))
{
    ...
    function get_filter_img($field)
    {
        ...
        {
            $html = '<a id="filter_icon_' . $field
                . '" onclick="delete_filter(\'' . $field
                . '\')" href="javascript:;" style="background: url(\''
                . static_base()
                . 'img/filter-delete.png\') 0 0 no-repeat; float: right; width: 12px; height: 11px; margin:2px 0px 0 5px;"></a>';
        } else
        {
            $html = '<a id="filter_icon_' . $field . '" onclick="open_filter(\''
                . $field . '\')" href="javascript:;" style="background: url(\''
                . static_base()
                . 'img/filter-icon.png\') 0 0 no-repeat; float: right; width: 12px; height: 11px; margin:2px 0px 0 5px;"></a>';
        }
...
if ( ! function_exists('get_linked_sort_img'))
{
    ...
    function get_linked_sort_img($field)
    {
        ...
        $html = '<a href="' . $link . '" style="background: url(\'' . base_url()
            . '/static/default/img/sort_icon.gif\') ' . $x_offset
            . 'px 0px no-repeat; width: ' . $width
            . 'px; height: 13px; float:right; margin-top:2px;"></a>';
...
if ( ! function_exists('share_url'))
{
    ...
    function share_url($platform)
    {
        ...
        {
            case 'zm':
                return 'http://link.apps.zing.vn/pro/view/conn/share?u='
                    . urlencode(full_url());
                break;
            case 'fb':
                ...
                break;
            case 'tw':
                return 'https://twitter.com/intent/tweet?url=' . full_url()
                    . '&text=my text there';
                break;
            case 'google':
...
if ( ! function_exists('alert'))
{
    ...
    function alert($message, $title = 'Thông báo', $type = 'error',
                   $is_echo = FALSE
    )
    {
        $alert
...
if ( ! function_exists('theme_web'))
{
    ...
    function theme_web()
    {
        return config_item('vpbase_theme');
    }
...
if ( ! function_exists('image_base_url'))
{

    function image_base_url()
    {
        return FVPBASE . 'media/images/';
    }

}

if ( ! function_exists('pure_url'))
{

    function pure_url()
    {
        $full_url = full_url();
        $url = explode("/index", $full_url);

        return $url[0] ? $url[0] : $full_url;
    }
...
if ( ! function_exists('script_global'))
{
    ...
    function script_global()
    {
        ...
        return '
            ...
            var template = "' . config_item('vpbase_theme') . '";
            var global_username = "' . $CI->session->userdata('user_username') . '";
            var rp_part_ext = "' . vp_config_item("rp_part_ext") . '";
        ';
    }
...
if ( ! function_exists('is_allow'))
{
    ...
    function is_allow($role_id, $resource_name, $type = 'r')
    {
        ...
        $CI->db->select('id');
        $resource = $CI->db->get_where(
            'module_resources', array('name' => $resource_name)
        );
...
if ( ! function_exists('is_admin'))
{
    ...
    function is_admin()
    {
        ...
        if ($CI->session->userdata('user_id')
            ...
            && count($CI->session->userdata('user_roles')) >= 1
        )
        {
            return TRUE;
        }
...
if ( ! function_exists('is_mod'))
{
    ...
    function is_mod()
    {
        ...
        if ($CI->session->userdata('user_id')
            ...
            && ($CI->session->userdata('user_user_role_id') == 2)
            && $CI->session->userdata('user_is_administrator') == 1
        )
        {
            return TRUE;
        }
    ...
    function pg_field_value($value, $type = 'TEXT', $module = NULL)
    {
        switch ($type)
        {
            case ConstFieldType::date:
                ...
                // get date
                return date(
                    config_item('date_frontend_format'), strtotime($value)
                );
                break;
            case ConstFieldType::datetime:
                ...
                // get date
                return date(
                    config_item('datetime_frontend_format'), strtotime($value)
                );
                break;
            case ConstFieldType::image:
                $small_thumb = get_image_thumb($value, 'small_thumb');
                if (is_file(FVPBASE . "media/images/$small_thumb"))
                {
                    $img = "<img class=\"img-responsive\" src='" . img_url()
                        . "media/images/$small_thumb' />";
                } else if (is_file(
                    FVPBASE . "media/images/" . get_image_thumb(
                        $value, 'medium_thumb'
                    )
                )
                )
                {
                    ...
                    $img = "<img class=\"img-responsive\" src='"
                        . img_url() . "media/images/$medium_thumb' />";
                } else if ($value == "")
                {
                    $img = "";
                } else
                ...
                return "<a class='pgImageField' href='" . img_url()
                    . "media/images/$value' rel='shadowbox'>$img</a>";
                break;
...
if ( ! function_exists('pagination_frontend'))
{
    ...
    function pagination_frontend($base_uri, $total_rows, $per_page = 10,
                                 $num_links, $extra_params = ''
    )
    {
        $CI = &get_instance();
        ...
        $config = array(
            ...
            'per_page' => $per_page,
            'num_links' => $num_links,
            'full_tag_open' => '<ul class="pagination-custom">',
            'full_tag_close' => '</ul>',
            ...
            //            'first_tag_open' => '<li>',
            //            'first_tag_close' => '</li>',
            'query_string_segment' => lang("trang")
        );
...
if ( ! function_exists('pagination_config'))
{
    ...
    function pagination_config($base_uri, $total_rows, $per_page = 20,
                               $segment = 5, $open_tag = '', $close_tag = '', $extra_params = ''
    )
    {
        $CI = &get_instance();
...
//        $config = array(
        ...
        $config = array(
            ...
            'next_tag_close' => '</li>',
            'last_link' => '>>',
            'last_tag_open' => '<li class="">',
            'last_tag_close' => '</li>',
            'first_link' => '<<',
            'first_tag_open' => '<li class="">',
            'first_tag_close' => '</li>',
        );
...
if ( ! function_exists('pagination_custom'))
{
    ...
    function pagination_custom($module, $controller, $action, $params,
                               $total_rows, $per_page = 20, $segment = 5, $extra_param = ''
    )
    {
        $CI = &get_instance();
        ...
        $config = array(
            ...
            'full_tag_open' => '<ul class="pagination">',
            'full_tag_close' => '</ul>',
            'cur_tag_open' => '<li class="paging_custom paging_active"><a href="javascript:void(0);"><span><span>',
            'cur_tag_close' => '</span></span></a></li>',
            'num_tag_open' => '<li class="paging_custom"><span><span>',
            'num_tag_close' => '</span></span></li>',
            'prev_link' => '<',
            'prev_tag_open' => '<li class="paging_custom"><span><span>',
            'prev_tag_close' => '</span></span></li>',
            'next_link' => '>',
            'next_tag_open' => '<li class="paging_custom"><span><span>',
            'next_tag_close' => '</span></span></li>',
            'last_link' => '>>',
            'last_tag_open' => '<li class="paging_custom"><span><span>',
            'last_tag_close' => '</span></span></li>',
            'first_link' => '<<',
            'first_tag_open' => '<li class="paging_custom"><span><span>',
...
if ( ! function_exists('pagination_ajax'))
{
    ...
    function pagination_ajax($module, $controller, $action, $params,
                             $total_rows, $per_page = 20, $segment = 5, $extra_param = ''
    )
    {
        $CI = &get_instance();
...
if ( ! function_exists('link_action'))
{
    ...
    function link_action($links, $ids = '', $class = 'action', $prefix = '')
    {
        ...
        if (is_array($links) && ! empty($links))
        {
            ...
            foreach ($links as $key => $link)
            {
                if (is_array($link))
                {
                    ...
                    {
                        $result_link .= '<a class="btn btn-default" href="'
                            . base_url($link['no_slash']) . $id . '" class="'
                            . $class . ' ' . $link['class'] . '" ' . $params
                            . '>' . lang($key) . '</a>';
                    } else
                    {
                        $params = '';
                        ...
                        $result_link .= '<a class="btn btn-default" href="'
                            . base_url($link['uri']) . '/' . $id . '?ReturnUrl='
                            . urlencode(full_url()) . '" class="' . $class . ' '
                            . $link['class'] . '" ' . $params . '>' . lang($key)
                            . '</a>';
                    }
                } else
                {
                    ...
                    if ($key == "CHARGE")
                    {
                        $result_link .= '<a class="btn btn-default" data-toggle="modal" data-target="#chargeform" onclick="charge('
                            . $id . ')">' . lang($key) . '</a>';
                    } else
                    {
                        $result_link .= '<a class="btn btn-default" href="'
                            . base_url($link) . '/' . $id . '?ReturnUrl='
                            . urlencode(full_url()) . '" class="' . $class
                            . '">' . lang($key) . '</a>';
                    }
        ...
        if ($links)
        {
            return '<a class="btn btn-default" href="' . base_url($links) . '/'
                . $id . '?ReturnUrl=' . urlencode(full_url()) . '" class="'
                . $class . '">' . lang('Edit') . '</a>';
        }
...
if ( ! function_exists('standar_date'))
{
    ...
    )
    {
        ...
        if ($show == TRUE)
        {
            ...
            $return_date = $date_array[2] . $char . $date_array[1] . $char
                . $date_array[0];
        ...
        } else
        {
            ...
            $return_date = $date_array[2] . $char_standar . $date_array[1]
                . $char_standar . $date_array[0];
...
if ( ! function_exists('datetime_format'))
{
    ...
    function datetime_reformat($date, $current_format = '')
    {
        if ($current_format != "")
        {
            ...
            return $date->format(config_item("datetime_system_format"));
        }

        return date(config_item("datetime_system_format"), strtotime($date));
    }
...
if ( ! function_exists('datetime_to_utc'))
{
    ...
    function datetime_to_utc($timestamp, $current_format = '')
    {
...
if ( ! function_exists('get_file_name'))
{
    ...
    {
        // remove _id
        $file_name = str_replace('_id', '', $file_name);
        ...
        {
            $extract_label = explode(".", $file_name);

            return $extract_label[0];
        }
    }
...
if ( ! function_exists('redirect_to'))
{
    ...
    function redirect_to($module, $controller = '', $action = 'index',
                         $params = NULL
    )
    {
        // get link
...
if ( ! function_exists('get_link'))
{
    ...
    )
    {
        ...
        $link = str_replace(
            '//', '/', "$r_module/$r_resource/$r_action/$params"
        );
...
if ( ! function_exists('get_folder_upload'))
{
    ...
    {
        // get dir path
        $dir = FVPBASE . 'media/' . $folder_uri;

        // get date
        $sub_folder = ($is_make_ymd_folder) ? date('Y') . '/' . date('m') . '/'
            . date('d') : '';
...
if ( ! function_exists('get_redim_folder_upload'))
{
    ...
    {
        // get dir path
        $dir = FVPBASE . 'media/uploads/' . $folder_uri;
...
if ( ! function_exists('get_image_thumb'))
{
    ...
    function get_image_thumb($image_uri, $thumb_maker)
    {
        ...
        {
            $image_thumb = $img_to_array[0] . '_' . $thumb_maker . '.'
                . $img_to_array[1];
        }
...
if ( ! function_exists('resize_image_minimize'))
{

    function resize_image_minimize($src_image_path, $config_options = array())
    {
        $CI = &get_instance();
...
if ( ! function_exists('resize_image'))
{
    ...
    function resize_image($src_image_path, $config_options = array())
    {
        ...
        if ($config['create_thumb'])
        {
            if ( ! $config['thumb_marker'])
            {
                $config['thumb_marker'] = '_' . $config['width'] . 'x'
                    . $config['height'];
            }
        }
...
if ( ! function_exists('upload_file'))
{
    ...
    function upload_file($field_name, $upload_uri, $type = 'jpg|JPG|jpge|JPGE',
                         $max_size = '2000', $max_width = '2048', $max_height = '2048',
                         $is_make_ymd_folder = TRUE
    )
    {
        $CI = &get_instance();
...
/**
 * Resize Thumb
 * modified 2011-11-30 9:50 by dungdv
 *
...
if ( ! function_exists('create_thumb'))
{
    ...
    function create_thumb($img_path, $width = 60, $height = 60,
                          $thumb_marker = '_small_thumb'
    )
    {
        $resize_config = array(
            'width' => $width,
...
if ( ! function_exists('minimize_image'))
{

    function minimize_image($img_path, $thumb_marker = '')
    {
        $resize_config = array(
            'thumb_marker' => $thumb_marker,
            'create_thumb' => TRUE,
        );
        resize_image_minimize($img_path, $resize_config);
    }
 ...
 *
 * @param string $string
 * @param string $remove_special // dungdv
 *
 * @return string
 */
if ( ! function_exists('make_slug'))
{
    ...
    function make_slug($string, $remove_special = FALSE)
    {
        ...
        if ($string)
        {
            ...
            $string = str_replace(
                $special_array, $remove_special, $string
            );
        }
...
if ( ! function_exists('draw_tree_category_block'))
{
    ...
    function draw_tree_category_block($category, $input_html, $level = 0,
                                      $indent_symbol = '-&nbsp;', $selected_value = array(), $href_uri = ''
    )
    {
        $output = '';
        foreach ($category['items'] as $cat)
        {
            ...
            $selected = (count($selected_value)
                && in_array(
                    $cat['id'], $selected_value
                )) ? 'selected' : '';
            ...
            if (isset($cat['items']))
            {
                $output .= draw_tree_category_block(
                    $cat, $input_html, $level + 1, $indent_symbol,
                    $selected_value, $href_uri
                );
            }
        }
...
/**
 * @author thangtpp
 * draw checkbox tree category
 *
 * @param array $category
 * @param integer $level
 * @param string $input_html
 *        example <input type="checkbox" value="##VALUE##" ##SELECTED##>##INDENT_SYMBOL##<label for="">##NAME##</label><br>
 * @param string $indent_symbol
 *        example '-&nbsp'
 * @param array $selected_value
 *
 * @return string
 */
if ( ! function_exists('draw_checkbox_tree_category_block'))
{

    function draw_checkbox_tree_category_block($category, $input_html,
                                               $level = 0, $indent_symbol = '-&nbsp;', $selected_value = array(),
                                               $href_uri = ''
    )
    {
        $output = '';
        foreach ($category['items'] as $cat)
        {
            ...
            if (isset($cat['items']))
            {
                $output .= draw_checkbox_tree_category_block(
                    $cat, $input_html, $level + 1, $indent_symbol,
                    $selected_value, $href_uri
                );
            }
...
if ( ! function_exists('print_captcha'))
{
    ...
    function print_captcha($width = '150', $height = '50', $length = 6)
    {
        $CI = &get_instance();
        $CI->load->helper('captcha');
        $CI->load->helper('strhash');

        if ( ! is_dir(FVPBASE . 'static/captcha/'))
        {
            mkdir(FVPBASE . 'static/captcha/', 0777, TRUE);
        }

        $vals = array(
            'word' => strtoupper(random_string($length)),
            'img_path' => FVPBASE . 'static/captcha/',
            'img_url' => base_url('static/captcha') . '/',
            'font_path' => FVPBASE . 'system/fonts/TAHOMA.TTF',
            'img_width' => $width,
            'img_height' => $height,
            'expiration' => 300,
        );
        ...
        $script_recaptcha = '<script>function js_recaptcha(){$.get("'
            . $link_recaptcha
            . '",function(data){$("#ci_captcha_image").html(data);});}'
            . '</script>';

        return '<div id="ci_captcha_image">' . $cap['image']
            . '<a href="#" class="JsReCaptcha" onclick="js_recaptcha();return false;"><span class="glyphicon glyphicon-repeat"></span></a></div><br />'
            . $script_recaptcha;
    }
...
if ( ! function_exists('print_captcha_without_recapt'))
{
    ...
    function print_captcha_without_recapt($width = '100', $height = '50',
                                          $length = 6
    )
    {
        $CI = &get_instance();
        ...
        return '<div id="ci_captcha_image" style="width:' . $width
            . 'px;height:' . $height . 'px; float:left">' . $cap['image']
            . '</div>';
    }
...
if ( ! function_exists('print_re_captcha'))
{
    ...
    function print_re_captcha($width = '150', $height = '50', $length = 6)
    {
        $CI = &get_instance();
        $CI->load->helper('captcha');
        $CI->load->helper('strhash');
        ...
        return $cap['image']
            . '<a href="#" class="JsReCaptcha" onclick="js_recaptcha();return false;"><span class="glyphicon glyphicon-repeat"></span></a>';
    }
...
if ( ! function_exists('check_captcha'))
{
    ...
    function check_captcha($captcha = '')
    {
        ...
        $CI->load->helper('strhash');
//        var_dump($captcha);
        //        var_dump(strtolower(string_hash($_SESSION['captcha'], false)));
        if ( ! empty($captcha))
        {
            if (strtolower($captcha) == strtolower(
                    string_hash($_SESSION['captcha'], FALSE)
                )
            )
            {
                return TRUE;
...
if ( ! function_exists('write_log_file'))
{
    ...
    function write_log_file($filename = 'error', $message = NULL)
    {
        ...
        write_file(
            FVPBASE . config_item('log_path') . $filename . '.log', $message,
            'a'
        );
    }
...
if ( ! function_exists('write_html_cache'))
{
    ...
    function write_html_cache($key, $output_cache)
    {
        ...
        $CI->load->helper('file');
        write_file(
            FVPBASE . APPPATH . "cache/html/cache__html__$key.html",
            $output_cache
        );
    }
...
if ( ! function_exists('get_html_cache'))
{
    ...
    {
        @include FVPBASE . APPPATH . "cache/html/cache__html__$key.html";
    }
        ...
        if (count($params) > 0)
        {
            ...
            foreach ($params as $key => $value)
            {
                if ($key != 's')
                {
                    if ($tempt == 0)
                    {
                        $return_extra_params = $return_extra_params . $key . '='
                            . $value;
                    } else
                    {
                        $return_extra_params = $return_extra_params . '&' . $key
                ...
                } else
                {
                    ...
                    foreach ($params_sort as $key_sort => $value_sort)
                    {
                        if ($tempt == 0)
                        {
                            $return_extra_params = $return_extra_params . 's['
                                . $key_sort . ']' . '=' . $value_sort;
                        } else
                        {
                            $return_extra_params = $return_extra_params . '&'
                                . 's[' . $key_sort . ']' . '=' . $value_sort;
                        }
...
if ( ! function_exists('generate_voting_params'))
{
    ...
    function generate_voting_params($resource_name, $record_id, $vote_type,
                                    $field_update_count, $point
    )
    {
        ...
        $token = sha1(
            $resource_name . $record_id . $vote_type . microtime(TRUE)
        );
        $_SESSION["sys_vt_{$resource_name}_{$record_id}_{$vote_type}"] = $token;

        $params_vote = string_hash(
            json_encode(
                array(
                    'record_id' => $record_id,
...
if ( ! function_exists('check_voting_params'))
{
    ...
    function check_voting_params($string_param_vote)
    {
        ...
        $params_vote = json_decode(
            string_hash($string_param_vote, FALSE), TRUE
        );

        if ($params_vote != NULL)
        {
            if (isset($params_vote['voting_token'])
                && $_SESSION["sys_vt_{$params_vote['resource_name']}_{$params_vote['record_id']}_{$params_vote['type_id']}"]
                == $params_vote['voting_token']
            )
            {
...
if ( ! function_exists('get_client_ip'))
{
    ...
    function get_client_ip()
    {
        ...
        {
            //to check ip is pass from gateway
...
if ( ! function_exists('remove_bad_words_in_comment'))
{
    ...
    function remove_bad_words_in_comment($comment)
    {
        ...
        $filter_badword = read_file(
            APPPATH . 'modules/comments/config/bad_words.txt'
        );
        ...
        $filter_sentence = read_file(
            APPPATH . 'modules/comments/config/bad_sentences.txt'
        );
...
if ( ! function_exists('template_url'))
{
    ...
    {
        return base_url() . 'static/rpgroup/';
    }

}

if ( ! function_exists('template_rp_url'))
{

    function template_rp_url()
    {
        return base_url() . 'static/rp_cus/';
    }
    ...
    function remove_directory($directory)
    {
        ...
        if (file_exists($directory))
        {
            foreach (glob("{$directory}/*") as $file)
            {
                if (is_dir($file))
                {
                    if (remove_directory($file) == FALSE)
                    {
                        $is_removed = FALSE;
                    }
                } else
...
// get config item with array element
if ( ! function_exists('vp_config_item'))
{

    function vp_config_item($config_item_name = "", $option = FALSE)
    {
        $config_item_data = config_item($config_item_name);
        if (empty($config_item_data))
        {
            return fasle;
        }
        if ($option !== FALSE)
        {
            return ($config_item_data[$option]) ? $config_item_data[$option]
                : FALSE;
        } else
        {
...
// get config item with array element
if ( ! function_exists('vp_file_exist'))
{

    function vp_file_exist($folder = "", $path = "")
    {
        return file_exists(FVPBASE . $folder . $path);
    }

}

// get config item with array element
if ( ! function_exists('vp_array_search'))
{

    function vp_array_search($rows, $field_name, $value)
    {
        $filtered = array();
        foreach ($rows as $row)
        {
...
// get config item with array element
if ( ! function_exists('vp_array_filtered'))
{

    function vp_array_filtered($rows, $field_name_search, $value_search,
                               $field_retrieved
    )
    {
//        debug($value_search);
        foreach ($rows as $row)
        {
            if ($row[$field_name_search] == $value_search)
            {
                return $row[$field_retrieved];
            }
        }
...
// get config item with array element
if ( ! function_exists('vp_number_format'))
{

    function vp_number_format($value = 0, $type = '', $exponent = 1,
                              $is_format = TRUE
    )
    {
//        debug($value_search);
        switch ($type)
        {
            ...
            case "in":
                //mm to inch
                $value = $value / pow(25.4, $exponent);
                break;
            default: //number, money
                break;
        }
...
// get config item with array element
if ( ! function_exists('vp_get_user_hash'))
{

    function vp_get_user_hash($user_identify = "")
    {
        $CI = &get_instance();
        $CI->load->helper('strhash');
        ...
        if ($user_identify == "")
        {
            $user_identify = $CI->session->userdata("user_id")
                . $CI->session->userdata("user_type");
        }

        if ($user_identify)
        {
            return "vp_" . md5($user_identify . "vp_hash_user_id");
        } else
        {
            return "not_found";
        }
...
// convertion rate from USD
if ( ! function_exists('vp_convert_currency'))
{

    function vp_convert_currency($value_in_USD = 0, $new_currency = "USD", $is_formated = FALSE)
    {

        if (strtoupper($new_currency) == "USD" || $new_currency == "")
        {
            $return_value = $value_in_USD;
        } else
        {
            $CI = &get_instance();
            $CI->load->model('products/Product_quote_configure');
            $currency_rate = $CI->Product_quote_configure->find("first_array", array(
                "where" => array(
                    "key" => strtoupper($new_currency),
                    "type" => "currency",
                    "status" => 1,
                )
            ));
            ...
            {
                $return_value = $value_in_USD * $currency_rate['value'];
            } else
            {
                $return_value = 0;
            }
        ...
        if ($is_formated)
        {
            $return_value = vp_format_currency($return_value, $new_currency);
        }
        return $return_value;
    }
...
// listing currencies
if ( ! function_exists('vp_list_currency'))
{

    function vp_list_currency()
    {
        $CI = &get_instance();
        $CI->load->model('products/Product_quote_configure');
        $currencies = $CI->Product_quote_configure->find("all", array(
            "where" => array(
                "type" => "currency",
                "status" => 1,
            )
...
// get rate
if ( ! function_exists('vp_exchange_rate'))
{

    function vp_exchange_rate($currency = '')
    {
        if ($currency == '')
        {
            return 1;
        ...
        $CI = &get_instance();
        $CI->load->model('products/Product_quote_configure');
        $currency_data = $CI->Product_quote_configure->find("first_array", array(
            "where" => array(
        ...
        if ( ! $currency_data && $currency_data['value'])
        {
            return 0;
        }
...
// listing currencies
if ( ! function_exists('vp_format_currency'))
{

    function vp_format_currency($value = 0, $currency = "")
    {
...
if ( ! function_exists('vp_get_file_icon'))
{

    function vp_get_file_icon($file_name)
    {
        $return = "fa-file-o";
...
if ( ! function_exists('vp_is_server_request'))
{
    function vp_is_server_request()
    {

        if ($_SERVER['SERVER_NAME'])
        {
            $serverIP = getHostByName($_SERVER['SERVER_NAME']);
            $clienIP = get_client_ip();
