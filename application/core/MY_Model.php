...
class MY_Model extends CI_Model {
     ...
     * @return Object / array
     */
    public function get($where = NULL, $order_by = NULL, $is_one = TRUE,
        $limit = 20, $offset = ''
    ) {
        $this->db->select('*');
     ...
     * @return Object / array
     */
    public function get_select($select, $where = NULL, $order_by = NULL,
        $is_one = TRUE, $limit = 20
    ) {
    ...
    public function create($dataAll, $check_field = FALSE)
    {
        ...
        $data['created']  = vp_mdate();
        $data['modified'] = vp_mdate();
        ...
        {
            write_log_file(
                "insert_database__{$this->session->userdata('user_username')}",
                "User: {$this->session->userdata('user_username')}, ip: {$this->input->ip_address()}, Table: {$this->db_table}, Data: "
                . json_encode($data)
            );
        }
    ...
    public function update($dataAll, $where, $check_field = FALSE)
    {
        ...
        {
            write_log_file(
                "update_database__{$this->session->userdata('user_username')}",
                "User: {$this->session->userdata('user_username')}, ip: {$this->input->ip_address()}, Table: {$this->db_table}, Where: "
                . json_encode($where) . ", Data: " . json_encode($data)
            );
        }
    ...
    public function delete($where, $is_restore = FALSE)
    {
        ...
        {
            write_log_file(
                "update_delete_database__{$this->session->userdata('user_username')}",
                "User: {$this->session->userdata('user_username')}, ip: {$this->input->ip_address()}, Table: {$this->db_table}, Data: "
                . json_encode($data)
            );
        }
    ...
    public function deleteRecord($where)
    {
        ...
        {
            write_log_file(
                "delete_database__{$this->session->userdata('user_username')}",
                "User: {$this->session->userdata('user_username')}, ip: {$this->input->ip_address()}, Table: {$this->db_table}, Data: "
                . json_encode($where)
            );
        }
    ...
    protected function log($level, $msg)
    {
        log_message(
            $level,
            __CLASS__ . '->' . __METHOD__ . ' :: ' . $msg . ' | In: ' . __FILE__
            . ' Line: ' . __LINE__
        );
    }
    ...
    public function is_duplicate($fieldName, $value)
    {
        ...
        } else
        {
            ...
            $query = $this->db->get_where(
                $this->db_table, array($fieldName => $value)
            );
            if ($query->num_rows > 0)
            {
                return TRUE;
...
//        else
    ...
    public function query($query, $no_result = FALSE)
    {
        ...
        {
            write_log_file(
                "query_database__{$this->session->userdata('user_username')}",
                "User: {$this->session->userdata('user_username')}, ip: {$this->input->ip_address()}, Table: {$this->db_table}, Data: "
                . json_encode($query)
            );
        }
     ...
     */
    public function getFormDataField($data, $resource_name = '',
        $is_main = FALSE
    ) {
        // get resource name
     ...
     * @return array
     */
    public function get_array($select, $where = NULL, $order_by = NULL,
        $is_one = TRUE, $limit = 20, $offset = 0
    ) {
     ...
     * @return array
     */
    private function _getTree($where = array(), $order = NULL, $parent_id = 0,
        ...
        if ($parents)
        {
            ...
            {
                $result[$i] = $parent;
                $childrens  = $this->_getTree(
                    $where, $order, $parent['id'], $field_parent
                );
                if ($childrens)
                {
     ...
     * @return array
     */
    public function getTreeItems($where = array(), $order = NULL,
        $parent_id = 0, $field_parent = 'parent_id'
    ) {
        return array('items' => $this->_getTree(
            $where, $order, $parent_id, $field_parent
        ));
    }
     ...
     */
    public function list_view_using_jpaging($page = 1, $page_size = 1,
        $controller = '', $action = '', $options = array()
    ) {
