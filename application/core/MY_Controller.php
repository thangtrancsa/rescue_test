...
class MY_Controller extends CI_Controller {
    ...
    function __construct()
    {
        ...
        if ($_COOKIE['timezoneOffset'])
        {
            $timezone_name = timezone_name_from_abbr(
                "", intval($_COOKIE['timezoneOffset']) * -60, 0
            );
        ...
        )
        {
            ...
            $cookie_config = array(
                ...
                'value' => $this->encryption->encode($_GET['language']),
                'expire' => '86400',
                'domain' => '',
                'path' => '/',
                'prefix' => '',
        ...
        if (ENVIRONMENT == 'production' && $this->session->userdata('user_id'))
        {
            if (is_admin())
            {
    ...
    protected function _filter($table)
    {
        ...
        if ($this->input->get('ReturnUrl') !== FALSE)
        {
            unset($filters['ReturnUrl']);
        }
        ...
        if ($filters)
        {
            ...
            {
                if (strpos($field, "custom__") !== FALSE)
                {
                    if ($field == 'created' || $field == 'modified' || $field == 'dob')
                    {
                        $this->paginator['like']['DATE(' . $table . $field . ')']
                            = standar_date(trim($filter), '-', '-');
                    }
                } else
                {
    ...
    )
    {
        ...
        if ($type_id == ARTICLE_TYPE)
        {
            ...
            $this->paginator['select']
                = 'articles.*, ad.subject as subject, ad.teaser as teaser, ad.tags as tags';

            $this->paginator['leftjoin']['article_dictionaries ad']
                = 'ad.article_id = articles.id';
            $this->paginator['join']['article_category_relationships acr']
                = 'acr.article_id = articles.id';

            $this->paginator['where']['articles.is_delete'] = 0;
            $this->paginator['where']['articles.is_publish'] = 1;
            $this->paginator['where']['articles.publish_date <='] = date(
                'Y-m-d H:i:s'
            );
        ...
        return array(
            ...
            'pages' => $this->getPaginationLink(
                $search_url, 5, $extra_params
            ),
        );
    }
    ...
    protected function _comment($resource, $record, $page_url = '', $limit = '')
    {
        ...
        return array(
            ...
            'pages' => ($page_url != '') ? $this->getPaginationLink($page_url)
                : NULL,
            'total' => $this->count_record,
    ...
    protected function _vote($resource_name, &$record, $type_id = '')
    {
        ...
        $vote = $this->Vote->find(
            "first_array", array(
                'select' => 'id',
                'where' => array('user_id' => $this->session->userdata(
                    'user_id'
                ), 'record_id' => $record['id']),
            )
        );
        ...
        } else
        {
            ...
            $vote_params = string_hash(
                "$record_id|$resource_name|$type_id|$token|$vote_count"
            );
     ...
     * @return boolean
     */
    protected function _increase_view($table_name, $record_id,
                                      $increase_number = ''
    )
    {
        //set session for user increase view in some time
        if (isset($_SESSION[SESS_VIEWED][$table_name][$record_id])
        ...
        $sql
            = "UPDATE $table_name SET $view_field = $view_field + $increase_number WHERE id = $record_id";
     ...
     * @return int 0 or 1
     */
    public function ajax_increase_view($table_name, $record_id,
                                       $increase_number = ''
    )
    {
        //set session for user increase view in some time
    ...
    {
        if ((COMMENT_NEED_LOGIN == TRUE && is_login())
            || COMMENT_NEED_LOGIN == FALSE
        )
        {
            //config 
            ...
            {
                $_SESSION[$session_id][$resource_id][$record_id][$user_id]
                    = array();
            }
    ...
    //list
    protected function _list($type_id, $cate_id, $page_url, $order = '',
                             $limit = '', $where_more = '', $except_ids = '', $focus = ''
    )
    {
        $this->paginator = array();
        ...
        if ($type_id == ARTICLE_TYPE)
        {
            ...
            $this->paginator['leftjoin']['article_dictionaries a']
                = "articles.id=a.article_id";
            $this->paginator['leftjoin']['article_category_relationships b']
                = "articles.id=b.article_id";
            $this->paginator['select']
                = "articles.id, thumbnail_image, is_hot, a.teaser, a.subject, publish_date, articles.counter_view";
            $this->paginator['where']['is_delete']
                = 0;
            $this->paginator['where']['is_publish']
                = 1;
            $this->paginator['where_wild']['publish_date <= ']
                = 'NOW()';
            $this->paginator['where']['b.article_category_id']
                = $cate_id;

            if ($where_more)
            {
                $this->paginator['where'] = array_merge(
                    $this->paginator['where'], $where_more
                );
            }

            $this->paginator['limit'] = $limit ? $limit
                : ARTICLE_RECORDS_PER_PAGE;
        ...
        if ($type_id == ARTICLE_TYPE)
        {
            if ($except_ids)
            {
                if (is_array($except_ids))
                {
                    ...
                    $this->paginator['where_not_in']['articles.id']
                        = $except_ids;
...
//                  debug($this->paginator);
    ...
    protected function _detail($type_id, $record_id)
    {
        ...
        if ($type_id == ARTICLE_TYPE)
        {
            $this->load->model("Articles/Article");
            $options['select']
                = 'articles.*, ad.subject as subject, ad.content as content, ad.teaser as teaser, ad.tags as tags';
            $options['join']
                = array('article_category_relationships acr' => 'acr.article_id = articles.id');
            $options['leftjoin']
                = array('article_dictionaries ad' => 'ad.article_id = articles.id');
            $options['where'] = array(
        ...
        {
            $this->load->model("Musics/Music");
            $options['where'] = array('id' => $record_id, 'status_id' => 1,
                'type_id' => MUSIC_TYPE_ID);
            $model_name = 'Music';
     ...
     * Redirec$this->PG_ACL('r');t về trang báo lỗi
     *
     * @param string $vpbase_permission r:read, w:write, e:modify, p:publish, d:delete, t:trash
     */
    public function PG_ACL($vpbase_permission = 'r')
    {
        $is_permission = $this->check_PG_ACL(
            $this->getUserRole(), $this->router->class, $vpbase_permission
        );
        if ($is_permission == FALSE)
        {
            if (strpos($this->router->class, 'admin') !== FALSE || $this->router->class == "dashboard")
            {
                if (is_login())
                {
     ...
     * @param string $vpbase_permission r:read, w:write, e:modify, p:publish, d:delete, t:trash
     */
    public function isACL($vpbase_permission = 'r')
    {
        return $this->check_PG_ACL(
            $this->getUserRole(), $this->router->class, $vpbase_permission
        );
    }
     ...
     *
     * @param        role               id $role_ids
     * @param int $resource_name controller name
     * @param string $vpbase_permission r:read, w:write, e:modify, p:publish, d:delete
     *
     * @return boolean
     */
    public function check_PG_ACL($role_ids, $resource_name,
                                 $vpbase_permission = 'r'
    )
    {
        ...
        {
            if (is_allow(
                $role_id["role_id"], $resource_name, $vpbase_permission
            )
            )
            {
                return TRUE;
    ...
    public function getUserRole()
    {
        ...
        {
            return $this->session->userdata('user_roles');
        } else
        {
     ...
     * @return array fields
     */
    public function getCustomField($user_id, $cfn_id = 0, $resource_name = '',
                                   $is_main = TRUE
    )
    {
        ...
        $this->db->join(
            'custom_field_names',
            'custom_fields.name_id = custom_field_names.id'
        );
        $this->db->join(
            'module_fields', 'custom_fields.field_id = module_fields.id'
        );
    ...
    public function getCustomFieldName($resource_name = '', $is_main = TRUE)
    {
        ...
        $cf_names = $this->db->get_where(
            'custom_field_names', array('resource_id' => $resource_id,
    ...
    public function getModuleName($resource_name = '', $module_id = 0)
    {
        ...
        {
            $this->db->select('name');
            $module = $this->db->get_where(
                'modules', array('id' => $module_id)
            );
        ...
        { // param is id
            $this->db->select('module_id');
            $resource = $this->db->get_where(
                'module_resources', array('id' => $resource_name)
            );
        } else
        { // param is name
            $this->db->select('module_id');
        ...
        $this->db->select('name');
        $module = $this->db->get_where(
            'modules', array('id' => $resource->row()->module_id)
        );
    ...
    public function getPaginationLink($uri, $segment = 5, $extra_params = '')
    {
        ...
        return pagination_config(
            $uri, $this->count_record, $limit, $segment, $open_tag = '',
            $close_tag = '', $extra_params
        );
    }
     ...
     * @return string
     */
    public function getPaginationCustom($module, $controller, $action, $params,
                                        $segment = 5, $is_ajax = FALSE, $extra_param = ''
    )
    {
        if (isset($this->paginator['limit']) && $this->paginator['limit'])
        {
        ...
        if ($is_ajax == TRUE)
        {
            return pagination_ajax(
                $module, $controller, $action, $params, $this->count_record,
                $limit, $segment, $extra_param
            );
        } else
        {
            return pagination_custom(
                $module, $controller, $action, $params, $this->count_record,
                $limit, $segment, $extra_param
            );
        }
     ...
     * @param $is_restore
     */
    public function deleteRecordOnListView($is_delete = FALSE,
                                           $is_restore = FALSE
    )
    {
        // get model name
        ...
        $link_redirect = $this->router->fetch_module() . '/'
            . $this->router->class;
        // add params redirect
        ...
        if ($this->input->post())
        {
            ...
            if (empty($list_delete_ids))
            {
                $this->session->set_flashdata(
                    'error_message', lang('Error params')
                );
            ...
            foreach ($list_delete_ids as $list_delete_id)
            {
                ...
                if ($is_delete == TRUE)
                {
                    ...
                    {
                        $this->$model->delete(
                            array('id' => $list_delete_id), TRUE
                        );
                    } else
                    { // delete record to recycle bin -> update is_delete = 1
                        $this->$model->delete(array('id' => $list_delete_id));
                    }
                } else
                { // not exit field is_delete -> delete record
                    $this->$model->deleteRecord(array('id' => $list_delete_id));
                }
            }

            $this->session->set_flashdata(
                'success_message', lang($model . 's deleted successful')
            );
            redirect($link_redirect);
        } else
        {
    ...
    private function _check_user_online()
    {
        ...
        $filepath = FVPBASE . 'application/cache/' . $filename;
    ...
    protected function _check_sso()
    {
        ...
        } else
        {
            ...
            if (empty($cookie_auth) || empty($cookie_uin)
                || strpos(
                    $cookie_uin, strval($_SESSION['user']['passport_id'])
                ) === FALSE
            )
            {
