...
class MY_Parser extends CI_Loader {
    ...
    public function parse($template, $data = array(), $return = FALSE,
        ...
        $template = CI::$APP->router->fetch_module() . '/views/' . $template;
     ...
     * @param mixed   $caching
     */
    public function parse_layout($template, $data = array(), $return = TRUE,
        $caching = TRUE
    ) {
        // VPBASE - @dungdv
        // make to path template
        $layout_view_dir   = $this->CI->layout->layout_dir;
        $smarty_layout_dir = substr(
            $layout_view_dir, 8, strlen($layout_view_dir)
        );
     ...
     */
    function string_parse($template, $data = array(), $return = FALSE,
        $is_include = FALSE
    ) {
