...
class MY_Pagination extends CI_Pagination {
    ...
    function create_links($show_number = TRUE, $s_between = '-')
    {
        ...
        if ($CI->config->item('enable_query_strings') === TRUE
            OR $this->page_query_string === TRUE
        )
        {
        ...
        {
            $first_url = ($this->first_url == '') ? $this->base_url
                : $this->first_url;
            $output    .= $this->first_tag_open . '<a ' . $this->anchor_class
                . 'href="' . $first_url . '">' . $this->first_link . '</a>'
        ...
        if ($this->prev_link !== FALSE AND $this->cur_page != 1)
        {
            ...
            if ($i == 0 && $this->first_url != '')
            {
                $output .= $this->prev_tag_open . '<a ' . $this->anchor_class
                    . 'href="' . $this->first_url . '">' . $this->prev_link
                    . '</a>' . $this->prev_tag_close;
            } else
            {
                $i      = ($i == 0) ? '' : $this->prefix . $i . $this->suffix;
                $output .= $this->prev_tag_open . '<a ' . $this->anchor_class
                    . 'href="' . $this->base_url . $i . '">' . $this->prev_link
                    . '</a>' . $this->prev_tag_close;
            }
            ...
            for ($loop = $start - 1; $loop <= $end; $loop++)
            {
                ...
                if ($i >= 0)
                {
                    if ($this->cur_page == $loop)
                    {
                        ...
                        if ($show_number == TRUE)
                        {
                            $output .= $this->cur_tag_open . $loop
                                . $this->cur_tag_close; // Current page
                        } else
                        {
                            $c_num   = ($loop - 1) * $this->per_page + 1;
                            $c_e_num = (($loop * $this->per_page)
                                < $this->total_rows) ? ($loop * $this->per_page)
                                : $this->total_rows;
                            $s_num   = $c_num . $s_between . $c_e_num;

                            $output .= $this->cur_tag_open . $s_num
                                . $this->cur_tag_close; // Current page
                        }
                    } else
                    {
                        ...
                        if ($n == '' && $this->first_url != '')
                        {
                            ...
                            if ($show_number == TRUE)
                            {
                                $output .= $this->num_tag_open . '<a '
                                    . $this->anchor_class . 'href="'
                                    . $this->first_url . '">' . $loop . '</a>'
                                    . $this->num_tag_close;
                            } else
                            {
                                $c_num   = ($loop - 1) * $this->per_page + 1;
                                $c_e_num = (($loop * $this->per_page)
                                    < $this->total_rows) ? ($loop
                                    * $this->per_page) : $this->total_rows;
                                $s_num   = $c_num . $s_between . $c_e_num;

                                $output .= $this->num_tag_open . '<a '
                                    . $this->anchor_class . 'href="'
                                    . $this->first_url . '">' . $s_num . '</a>'
                                    . $this->num_tag_close;
                            }
        ...
        {
            $output .= $this->next_tag_open . '<a ' . $this->anchor_class
                . 'href="' . $this->base_url . $this->prefix . ($this->cur_page
                    * $this->per_page) . $this->suffix . '">' . $this->next_link
        ...
        )
        {
            ...
            $output .= $this->last_tag_open . '<a ' . $this->anchor_class
                . 'href="' . $this->base_url . $this->prefix . $i
                . $this->suffix . '">' . $this->last_link . '</a>'
                . $this->last_tag_close;
        }
