...
class Layout {
    ...
    private $layout_activedMenu = 'MainActivedMenu';
    ...
    public function output()
    {
        ...
        // does layout exist
        if ($CI->layout->active AND $CI->layout->layout AND file_exists(
                APPPATH . $CI->layout->layout_dir . $CI->layout->layout . '.tpl'
            )
        )
        {
            // return layout
            $buffer = $CI->parser->parse_layout(
                $CI->layout->layout, array(
                ...
                $CI->layout->layout_activedMenu => $CI->layout->get_activedMenu(
                ),
        ...
        if ($CI->layout->clean_output)
        {
            ...
            $search = array(
                '/\>[^\S \t]+/s',
                // strip whitespaces after tags, except space and tab
                '/[^\S ]+\</s', // strip whitespaces before tags, except space
                '/(\s)+/s', // shorten multiple whitespace sequences
                '#\/\/\<\!\[CDATA\[[^\S ]+#s'
                // strip whitespace directly after CDATA
            );
...
//        debug($this->pgJavascript);
    ...
    public function get_title()
    {
        if ($this->title)
        {
            return $this->title . ' - ' . config_item('vpbase_site_name');
        } else
        {
            return config_item('vpbase_site_name');
        }
    }
    ...
    public function get_image()
    {
        ...
        {
            return config_item('vpbase_site_image');
        }
    }
    ...
    public function get_keyword()
    {
        ...
        {
            return config_item('vpbase_site_keyword');
        }
    }
    ...
    public function get_description()
    {
        ...
        {
            return config_item('vpbase_site_description');
        }
    }
     ...
     * @return activedMenu
     */
    public function set_activedMenu($activedMenu)
    {
     ...
     * @return activedMenu
     */
    public function get_activedMenu()
    {
        return $this->activedMenu;
    }
        ...
        if ($this->pgRel)
        {
            ...
            if (is_array($this->pgRel))
            {
                ...
                foreach ($this->pgRel as $key => $css)
                {
                    if (is_numeric($key))
                    {
                        $style .= '<link href="' . $this->getLinkStatic(
                                'css', $css
            ...
            {
                // return string add css to view
                return '<link href="' . $this->getLinkStatic(
                        'css', $this->pgRel
                    ) . '" rel="stylesheet" type="text/css" />';
            }
        }
        ...
        if ($this->pgJavascript)
        {
                ...
                foreach ($this->pgJavascript as $key => $js)
                {
                    if (is_numeric($key))
                    {
                        $javascript .= '<script type="text/javascript" src="'
                            . $this->getLinkStatic('js', $js) . '"></script>';
                    } else
                    {
                        $javascript .= '<script type="text/javascript" src="'
                            . $this->getLinkStatic($key, $js) . '"></script>';
                    }
                }
            ...
            } else // là chuỗi thì add 1 file javascript đến view
            {
                ...
                return '<script type="text/javascript" src="'
                    . $this->getLinkStatic('js', $this->pgJavascript)
                    . '"></script>';
            }
        }
    ...
    {
        // link statis
        $link = static_url() . 'static/';

        if ($subfolder == 'css' || $subfolder == 'js')
        {
            $link .= theme_web() . '/';
        }
